...
function Timesheet() {
  ...
  useEffect(() => {
    getTimesheets({ sortBy: orderBy, order, ...selectedFilters }).catch(
      (err) => {
  ...
  const getData = async () => {
    ...
    const rows = timesheets.map((ts: any, index: number) => {
      const group = ts.group; // groups.find((g: any) => g.id === ts.group.id);

      return {
        employeeId: ts.employeeId,
        hasNonAcceptableBreaks:
          ts.hasNonAcceptableBreaks || ts.hasMalfunction ? (
            <WarningRoundedIcon sx={{ color: "#eed202" }} />
          ) : (
            ""
          ),
        name: ts.name,
        nameLink: (
          <Link
            sx={{ cursor: "pointer" }}
            onClick={handleClickEmployeeLink(ts.employeeId)}
          >
            {ts.name}
          </Link>
        ),
        ...
        edit: (
          <>
            {group && (
              <Tooltip key={index} title="Edit Times">
    ...
    const collapsedRows = timesheets.map((ts: any, index: number) => {
      return ts.breaks.map((b: any, index2: number) => ({
        ...
        endTime: b.endTime,
        duration:
          !ts.hasMalfunction && b.duration !== "" && b.minsExceeding !== ""
            ? `${b.duration} (${b.minsExceeding > 0 ? "+" : ""}${
                b.minsExceeding
              })`
            : "",
  ...
  const editRows =
    (index: number) =>
    (key: string) =>
    (newValue: any, keyboardInputValue: any) => {
      setRows((rows) => {
        if (rows === null) return null;
        rows[index][key] = keyboardInputValue
          ? keyboardInputValue
          : newValue
  ...
  const editCollapsedRows =
    (index: number) =>
    (row: number, key: string) =>
    (newValue: any, keyboardInputValue: any) => {
      setCollapsedRows((collapsedRows) => {
        if (collapsedRows === null) return null;
        collapsedRows[index][row][key] = keyboardInputValue
          ? keyboardInputValue
          : newValue
          ? format(newValue, "HH:mm")
          : "";
  ...
  const handleSave = async () => {
    if (selectedRow !== undefined && rows) {
      const data = {
        ...
        breaks: collapsedRows
          ? collapsedRows[selectedRow].map((el: any) => ({
  ...
  const handleDeleteCollapsedRow = () => {
    ...
    if (selectedRow === undefined)
      setSelectedRow(Number(anchorEl.getAttribute("data-row1")));

    setCollapsedRows((collapsedRows) => {
      if (collapsedRows === null) return null;

      // @ts-ignore
      collapsedRows[anchorEl.getAttribute("data-row1")].splice(
        anchorEl.getAttribute("data-row2"),
        1
      );
      ...
      for (
        let i = 0;
        i < collapsedRows[anchorEl.getAttribute("data-row1")].length;
        i++
      ) {
        // @ts-ignore
        collapsedRows[anchorEl.getAttribute("data-row1")][`${i}`].options = (
          <IconButton
            data-row1={anchorEl.getAttribute("data-row1")}
            data-row2={`${i}`}
            onClick={handleMoreClick}
          >
            <MoreVertIcon />
    ...
    if (rows)
      setSelectedEmployee({
        id: rows[Number(anchorEl.getAttribute("data-row1"))].employeeId,
        name: "",
      });
  ...
  const handleAddRow = (isAbove: boolean) => () => {
    ...
    if (isAbove) {
      // @ts-ignore
      collapsedRows[anchorEl.getAttribute("data-row1")].splice(
        Number(anchorEl.getAttribute("data-row2")),
        0,
        { ...newItem }
      );
    } else {
      // @ts-ignore
      collapsedRows[anchorEl.getAttribute("data-row1")].splice(
        Number(anchorEl.getAttribute("data-row2")) + 1,
        0,
        { ...newItem }
      );
    }
  ...
  const addCollapsedRow = (index: number) => () => {
    ...
    const newItem = {
      ...
      options: (
        <IconButton
          data-row1={`${index}`}
          data-row2={`${collapsedRows[index].length}`}
          onClick={handleMoreClick}
        >
          <MoreVertIcon />
        </IconButton>
  ...
  return (
    <>
        ...
        <div style={{ display: "flex", gap: "4px" }}>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <DesktopDatePicker
              ...
              renderInput={({ error, onError, ...params }) => (
                <TextField sx={{ minWidth: "150px" }} {...params} />
              )}
              disableFuture={true}
            />
          </LocalizationProvider>
          {displayedFilters.map((e: any) => (
            <FilterSelectList
              id={e.id}
              label={e.label}
              getData={e.getData}
              editFilter={editFilter(e.filterId)}
              value={e.value}
      ...
      >
        {listedFilters.map((f) => (
          <MenuItem onClick={handleClickListedFilter(f.id)}>{f.label}</MenuItem>
        ))}
